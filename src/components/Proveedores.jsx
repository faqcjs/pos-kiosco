...
export default function Proveedores() {
  const { proveedores, agregarProveedor, registrarRecepcionMercaderia, registrarPagoProveedor, eliminarProveedor, cajaActiva, mostrarNotificacion } = useKioskoStore()
  ...
  const SupplierLedgerDetails = ({ supplier, onClose }) => {
    ...
    return (
      <div className="space-y-5 animate-fade-in text-slate-350">
        <div className="flex justify-between items-start border-b border-slate-800 pb-4 bg-[#141926] p-4 rounded-t-2xl">
          <div>
            <h3 className="text-lg font-black text-slate-100">{supplier.nombre}</h3>
        ...
        <div className="px-4 pb-4 space-y-5">
          <div className="bg-amber-500/10 border-2 border-amber-500/25 rounded-2xl p-4 text-center max-w-[220px] mx-auto">
            <span className="text-[10px] text-amber-400 font-black block uppercase tracking-wider">
              Nuestra Deuda Total
            </span>
            <span className="text-3xl font-black text-amber-450 mt-1 block">
          ...
          <div className="border-t border-slate-800/80 pt-4">
            <h4 className="font-extrabold text-slate-200 text-xs uppercase tracking-wider mb-3">
            ...
            <form onSubmit={handleRecepcion} className="space-y-3">
              <div className="flex gap-2">
                <div className="relative flex-grow">
                  ...
                  <input
                    ...
                    className="w-full bg-[#090b11] border border-slate-800 rounded-xl py-2.5 pl-6.5 pr-2 text-xs outline-none focus:border-indigo-500 text-slate-200 font-bold"
                    required
                  />
                </div>
                <input
                  ...
                  className="flex-[1.5] bg-[#090b11] border border-slate-800 rounded-xl py-2.5 px-3 text-xs outline-none focus:border-indigo-500 text-slate-200"
                />
                <button
                  type="submit"
                  className="bg-amber-500 hover:bg-amber-600 text-white font-extrabold px-4 py-2.5 rounded-xl text-xs shadow-md shadow-amber-950/20 transition-all btn-interactive shrink-0"
                >
                  Registrar
                </button>
              </div>
              ...
              <label className="flex items-center gap-2 text-[11px] text-slate-500 select-none pl-0.5">
                <input
                  type="checkbox"
                  checked={recepcionPagada}
                  onChange={(e) => setRecepcionPagada(e.target.checked)}
                  className="rounded bg-[#090b11] border-slate-800 text-amber-500 focus:ring-amber-550 h-3.5 w-3.5"
                />
                <span>¿Pagar en el momento con efectivo de la caja chica?</span>
              </label>
              {recepcionPagada && !cajaActiva && (
                <p className="text-[10px] text-rose-450 pl-0.5 animate-pulse">
              ...
              <form onSubmit={handlePago} className="space-y-3">
                <div className="flex gap-2">
                  <div className="relative flex-grow">
                    ...
                    <input
                      ...
                      className="w-full bg-[#090b11] border border-slate-800 rounded-xl py-2 pl-6 pr-2 text-xs font-bold text-slate-200 outline-none focus:border-emerald-500"
                      required
                    />
                  </div>
                  <button
                    type="submit"
                    className="bg-emerald-600 hover:bg-emerald-500 text-white font-extrabold px-4 py-2 rounded-xl text-xs shadow-md shadow-emerald-950/20 transition-all btn-interactive"
                  >
                    Pagar
                  </button>
                </div>
                ...
                <label className="flex items-center gap-2 text-[11px] text-slate-500 select-none">
                  <input
                    ...
                    className="rounded bg-[#090b11] border-slate-800 text-indigo-500 focus:ring-indigo-550 h-3.5 w-3.5"
                  />
          ...
          <div className="border-t border-slate-800/80 pt-4">
            <h4 className="font-extrabold text-slate-200 text-xs uppercase tracking-wider mb-3">Historial de Transacciones</h4>
                  ...
                  >
                    ...
                    <span
                      className={`font-black shrink-0 ml-3 text-sm ${
                        h.tipo === 'recepcion' ? 'text-amber-400' : 'text-emerald-450'
                      }`}
                    >
          ...
          <div className="border-t border-slate-800/80 pt-4 flex gap-3">
            <button
              ...
              className="flex-1 py-3 bg-rose-500/10 hover:bg-rose-500/25 border border-rose-500/20 text-rose-450 font-bold rounded-xl text-xs uppercase tracking-wider transition-all btn-interactive text-center"
            >
              🗑️ Borrar Proveedor
            </button>
          </div>
    ...
    <div className="flex-1 flex flex-col md:flex-row overflow-hidden h-full pb-16 md:pb-0 bg-[#090b11]">
                  ...
                  <div className="text-right shrink-0">
                    <span
                      className={`text-base font-black block leading-none ${
                        p.deuda > 0 ? 'text-amber-400' : 'text-slate-500'
                      }`}
                    >
      ...
      <div className="hidden md:flex w-[480px] bg-[#10141f] flex-col overflow-y-auto">
        {proveedorSeleccionado ? (
          <SupplierLedgerDetails supplier={proveedorSeleccionado} />
        ) : (
      ...
      {modalNuevoAbierto && (
        <>
          ...
          <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md bg-[#10141f] border border-slate-800 rounded-t-3xl z-50 px-5 pb-8 pt-5 animate-slide-up shadow-2xl">
            <div className="w-10 h-1 bg-slate-700 rounded-full mx-auto mb-5" />
            <h3 className="text-center text-lg font-bold text-slate-300 uppercase tracking-widest mb-4">
              Registrar Proveedor
            </h3>
