...
export default function NotificacionModal() {
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black/75 z-999 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in">
        <div
          className={`bg-[#10141f] border ${getBorderColor()} rounded-3xl p-6 max-w-sm w-full text-center space-y-4 shadow-2xl animate-slide-up`}
        >
          {/* Icono */}
          ...
          <div className="flex gap-3 pt-2">
            {alCancelar && (
              <button
                ...
                className="flex-1 py-3 bg-slate-800 active:bg-slate-750 hover:bg-slate-700/60 text-slate-300 text-xs font-bold rounded-2xl transition-all btn-interactive"
              >
                Cancelar
              </button>
            )}
